...
-- 2. 重新创建dijkstra_metro_shortest_path函数（修复列引用歧义）
...
-- 3. 重新创建calculate_metro_commute_time函数（确保没有列引用歧义）
...
COMMENT ON FUNCTION public.calculate_metro_commute_time(text, text) IS '计算两个地铁站之间的通勤时间，使用Dijkstra算法找到最优路线（已修复列引用歧义问题）';

-- 6. 验证修复
SELECT '全面修复完成' as status, NOW() as fix_time;
