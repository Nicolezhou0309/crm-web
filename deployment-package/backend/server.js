...
async function handleWecomUserInDatabase(userInfo) {
  try {
    ...
    const { UserId, email, corpId } = userInfo;
    ...
    const jwtSecret = process.env.VITE_SUPABASE_JWT_SECRET || 'your-jwt-secret';
...
app.get('/api/auth/wecom/callback', async (req, res) => {
  try {
    ...
    if (userId) {
      ...
      // 非企业成员：构建基础信息
      console.log(`👤 处理非企业成员信息`);
      userInfo = {
        UserId: externalUserId || openId, // 使用external_userid或openid作为标识
        email: `${openId}@external.wecom.local`,
        corpId: WECOM_CONFIG.corpId
      };
    }
