-- 修复入住时间筛选函数，添加非空条件支持
-- 问题：当前函数允许入住时间为NULL的记录通过筛选
-- 解决方案：添加 p_moveintime_not_null 参数，并修改筛选逻辑

-- 1. 删除现有函数（如果存在）
DROP FUNCTION IF EXISTS public.filter_followups(
    p_created_at_end timestamp with time zone,
    p_created_at_start timestamp with time zone,
    p_customerprofile customerprofile[],
    p_followupresult text[],
    p_followupstage followupstage[],
    p_interviewsales_user_id bigint[],
    p_leadid text[],
    p_leadtype text[],
...
BEGIN
    ...
    WHERE ($7 IS NULL OR f.leadid = ANY($7))
      ...
      AND ($13 IS NULL OR f.moveintime IS NOT NULL)  -- 🆕 修复：直接使用参数而不是变量
      AND ($22 IS NULL OR f.userrating = ANY($22) OR (ARRAY_LENGTH($22, 1) = 1 AND $22[1] IS NULL AND f.userrating IS NULL))
      AND ($10 IS NULL OR (
        CASE 
      ...
      AND ($23 IS NULL OR l.wechat = ANY($23) OR (ARRAY_LENGTH($23, 1) = 1 AND $23[1] IS NULL AND l.wechat IS NULL))
    ...
    EXECUTE v_count_query INTO v_total_count USING 
        ...
        p_moveintime_start, p_moveintime_not_null, p_offset, p_remark, p_scheduledcommunity, p_showingsales_user,
...
-- 3. 验证函数是否创建成功
-- SELECT routine_name, routine_type FROM information_schema.routines WHERE routine_name = 'filter_followups';

-- 4. 测试函数是否正常工作
-- SELECT * FROM public.filter_followups(
--     p_moveintime_not_null := ARRAY[true]
-- ) LIMIT 5;
