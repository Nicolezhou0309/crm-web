-- 先删除原有函数（参数类型要完全一致）
DROP FUNCTION IF EXISTS public.filter_leads(
  text, timestamp with time zone, timestamp with time zone, timestamp with time zone, timestamp with time zone,
  text, text, text, text, text, text, source, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text
);

-- 再创建新版（带 interviewsales 字段）
