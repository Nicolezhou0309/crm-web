-- 部署筛选选项获取函数
-- 这个函数用于获取动态字段的筛选选项，支持手机号和微信号的脱敏处理

\echo '正在部署 get_filter_options 函数...'
...
\echo 'get_filter_options 函数部署完成！'

-- 测试函数是否正常工作
\echo '测试函数...'
SELECT * FROM get_filter_options('phone') LIMIT 5;
SELECT * FROM get_filter_options('wechat') LIMIT 5;
SELECT * FROM get_filter_options('leadid') LIMIT 5;
