-- 修复 filter_followups 函数，添加预约时间筛选逻辑
-- 解决预约时间筛选器无法正常工作的问题
...
CREATE OR REPLACE FUNCTION public.filter_followups(
    ...
    p_scheduletime_not_null boolean[] DEFAULT NULL::boolean[],
...
DECLARE
    ...
    v_scheduletime_not_null boolean; -- 🆕 新增：预约时间非空条件变量
BEGIN
    -- 处理入住时间非空条件
    ...
    -- 🆕 新增：处理预约时间非空条件
    v_scheduletime_not_null := FALSE;
    IF p_scheduletime_not_null IS NOT NULL AND array_length(p_scheduletime_not_null, 1) > 0 THEN
        v_scheduletime_not_null := p_scheduletime_not_null[1];
    END IF;
    ...
    WHERE (p_created_at_end IS NULL OR f.created_at <= p_created_at_end)
      ...
      -- 🆕 新增：预约时间筛选逻辑
      AND (p_scheduletime_end IS NULL OR f.scheduletime <= p_scheduletime_end)
      AND (p_scheduletime_start IS NULL OR f.scheduletime >= p_scheduletime_start)
      AND (v_scheduletime_not_null IS NULL OR (CASE WHEN v_scheduletime_not_null THEN f.scheduletime IS NOT NULL ELSE TRUE END))
...
FROM pg_proc 
WHERE proname = 'filter_followups';
...
FROM pg_proc p
WHERE p.proname = 'filter_followups';
