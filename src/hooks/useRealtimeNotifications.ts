...
export const useRealtimeNotifications = () => {
  ...
  useEffect(() => {
    ...
    console.log('[useRealtimeNotifications] 查询用户档案:', { userId: user.id, userType: typeof user.id });
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          console.error('[useRealtimeNotifications] 查询用户档案失败:', error);
          setProfileId(null);
        } else {
          const pid = data?.id || null;
          console.log('[useRealtimeNotifications] 查询用户档案成功:', { profileId: pid, data });
          setProfileId(pid);
          // 缓存profileId
          if (pid) {
