-- 最终修复 filter_followups 函数的参数顺序和数量问题
-- 问题：参数数量不匹配，p_worklocation位置错误
...
BEGIN
    ...
    -- 🆕 修复：构建WHERE子句，使用正确的参数顺序和数量
    v_where_clause := '
    WHERE ($1 IS NULL OR f.created_at <= $1)
      ...
      AND ($7 IS NULL OR f.leadid = ANY($7))
      AND ($8 IS NULL OR f.leadtype = ANY($8) OR (ARRAY_LENGTH($8, 1) = 1 AND $8[1] IS NULL AND f.leadtype IS NULL))
      AND ($9 IS NULL OR f.majorcategory = ANY($9) OR (ARRAY_LENGTH($9, 1) = 1 AND $9[1] IS NULL AND f.majorcategory IS NULL))
      AND ($10 IS NULL OR (
        CASE 
      ...
      AND ($14 IS NULL OR f.remark ILIKE ''%'' || $14 || ''%'' OR ($14 = '''' AND (f.remark IS NULL OR f.remark = '''')))
      ...
      AND ($44 IS NULL OR l.wechat = ANY($44) OR (ARRAY_LENGTH($44, 1) = 1 AND $44[1] IS NULL AND l.wechat IS NULL))
    ...
    -- Count total records - 使用正确的参数顺序和数量（45个参数）
    ...
    EXECUTE v_count_query INTO v_total_count USING 
        ...
        p_interviewsales_user_id, p_leadid, p_leadtype, p_majorcategory, p_moveintime_end,
        p_moveintime_start, v_moveintime_not_null, p_remark, p_remark, p_scheduledcommunity, p_showingsales_user,
        p_source, p_userbudget, p_userbudget_min, p_userbudget_max, p_userrating, p_worklocation,
        ...
        p_creativedid, p_creativename, p_traffictype, p_interactiontype, p_douyinleadid, p_leadstatus, p_wechat, p_keyword;
    ...
    -- Execute query and return results - 使用正确的参数顺序和数量（45个参数）
