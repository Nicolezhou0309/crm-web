...
BEGIN
  ...
  IF array_length(p_row_fields, 1) > 0 THEN
    ...
    LOOP
      -- 验证字段是否存在
      BEGIN
        EXECUTE 'SELECT ' || quote_ident(v_row_field) || ' FROM filter_all_analysis_multi() LIMIT 1' INTO v_debug_info;
        v_select_fields := v_select_fields || quote_ident(v_row_field) || ', ';
        RAISE NOTICE '行维度字段 % 验证通过', v_row_field;
      EXCEPTION
        WHEN OTHERS THEN
          RAISE NOTICE '警告: 行维度字段 % 可能不存在: %', v_row_field, SQLERRM;
          -- 继续执行，让SQL错误自然发生
          v_select_fields := v_select_fields || quote_ident(v_row_field) || ', ';
      END;
    END LOOP;
  ...
  IF array_length(p_column_fields, 1) > 0 THEN
    ...
    LOOP
      -- 验证字段是否存在
      BEGIN
        EXECUTE 'SELECT ' || quote_ident(v_col_field) || ' FROM filter_all_analysis_multi() LIMIT 1' INTO v_debug_info;
        v_select_fields := v_select_fields || quote_ident(v_col_field) || ', ';
        RAISE NOTICE '列维度字段 % 验证通过', v_col_field;
      EXCEPTION
        WHEN OTHERS THEN
          RAISE NOTICE '警告: 列维度字段 % 可能不存在: %', v_col_field, SQLERRM;
          -- 继续执行，让SQL错误自然发生
          v_select_fields := v_select_fields || quote_ident(v_col_field) || ', ';
      END;
    END LOOP;
  ...
  LOOP
    -- 验证聚合字段是否存在
    BEGIN
      EXECUTE 'SELECT ' || quote_ident(v_value_field->>'field') || ' FROM filter_all_analysis_multi() LIMIT 1' INTO v_debug_info;
      RAISE NOTICE '聚合字段 % 验证通过', v_value_field->>'field';
    EXCEPTION
      WHEN OTHERS THEN
        RAISE NOTICE '警告: 聚合字段 % 可能不存在: %', v_value_field->>'field', SQLERRM;
    END;
...
COMMENT ON FUNCTION execute_pivot_analysis IS '执行透视表分析，支持动态SQL构建和枚举字段筛选条件处理 - 支持新的联合分析函数版本'; 