...
-- 修复透视表函数的GROUP BY错误
-- 修复时间: 2025年1月15日
-- 问题: 动态列字段未包含在GROUP BY子句中
-- =============================================

-- 重新创建多层级透视表函数，修复GROUP BY错误
    ...
    FOR i IN 0..jsonb_array_length(p_filters) - 1 LOOP
      ...
      IF v_filter_value LIKE '%,%' AND v_filter_operator = 'contains' THEN
        ...
        FOR j IN 1..array_length(v_filter_values, 1) LOOP
          ...
          END IF;
          v_dynamic_columns := v_dynamic_columns || 
            'MAX(CASE WHEN ' || quote_ident(v_filter_field) || ' ILIKE ' || 
...
COMMENT ON FUNCTION execute_multi_level_pivot_analysis IS '多行表头透视表分析函数，支持类似Excel的多行表头效果 - 修复GROUP BY错误版本'; 