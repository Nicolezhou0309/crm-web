-- 删除旧版本的 filter_deals 函数，解决函数冲突问题

-- 删除所有版本的 filter_deals 函数
DROP FUNCTION IF EXISTS public.filter_deals(
  uuid[], text[], date, date, uuid[], text[], text[], text[], timestamp with time zone, timestamp with time zone, text[]
);

DROP FUNCTION IF EXISTS public.filter_deals(
  uuid[], text[], date, date, uuid[], text[], text[], text[], timestamp with time zone, timestamp with time zone, text[], text, boolean, integer, integer
);

-- 重新创建正确版本的 filter_deals 函数
