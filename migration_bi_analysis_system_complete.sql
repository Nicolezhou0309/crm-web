...
COMMENT ON FUNCTION get_user_pivot_configs IS '获取用户透视表配置';
...
VALUES (
  ...
  '{
    "rowFields": ["source"],
    "columnFields": ["followupstage"],
    "valueFields": [
      {"field": "leadid", "aggregation": "count_distinct"},
      {"field": "userbudget", "aggregation": "avg"}
    ],
    "filters": [],
    "sortBy": []
  }'::jsonb,
...
-- 检查用户是否有权限访问BI功能
CREATE OR REPLACE FUNCTION check_bi_permission()
RETURNS boolean
LANGUAGE plpgsql
AS $$
DECLARE
...
COMMENT ON FUNCTION check_bi_permission IS '检查用户是否有BI功能权限';
...
SELECT '包含以下功能：' as feature_list;
SELECT '- 透视表配置管理' as feature1;
SELECT '- 多数据源支持' as feature2;
SELECT '- 一对多关系处理' as feature3;
SELECT '- 灵活的聚合方式' as feature4;
SELECT '- 行级安全策略' as feature5; 